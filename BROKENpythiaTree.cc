...
{ 
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) 
	...
	{  // loop over all particles in the event
	  ...
	  if ( abs( pythia.event[i].id() ) == 4900101) 
	      ...
	      if(abs(pythia.event[i].status())==71) 
		{
		  ndq71++;
		  if(idbg>0) 
		    cout<<" dark quark with code 71 is "<<i<<" "<<pythia.event[i].pT()<<" "<<pythia.event[i].y()<<" "<<pythia.event[i].phi()<<endl;
		  if(dq711==0) 
		    {
		      dq711=i;
		    } 
	  ...
	  if(abs(pythia.event[i].id())>4900000) 
	    {
	      ...
	      if (ndauHV<2) 
		hstatus2->Fill(pythia.event[i].status());
	      hndau->Fill(ndauHV);
	      hndau2->Fill(abs(HV),ndauHV);
	      
	      //Creat Vector of Dark Pion indexes
	      if( idHV == 4900111)
		{
		  Dvpions[counter] = i;
		  counter++;
	      ...
	      if(HV==21) 
		{
		  if(pythia.event[i].daughter1()!=0) 
		    hppid2ddg->Fill(pythia.event[pythia.event[i].daughter1()].id()-4900000);
		  if(pythia.event[i].daughter2()!=0) 
		    hppid2ddg->Fill(pythia.event[pythia.event[i].daughter2()].id()-4900000);
		}
	      if(ndauHV>0) 
		{ // if it is not a stable HV particle
	...
	hjet1pT->Fill(aSlowJet.pT(0));
      if(aSlowJet.sizeJet()>1)  
	hjet2pT->Fill(aSlowJet.pT(1));
      if(aSlowJet.sizeJet()>2)  
	hjet3pT->Fill(aSlowJet.pT(2));
      if(aSlowJet.sizeJet()>3)  
	hjet4pT->Fill(aSlowJet.pT(3));
      ...
      for (int ii =0; ii< ndpis; ++ii) 
	{
      ...
      for ( int wip = 0; wip < Dvpions.size(); wip++)
	{
	  //Grab the Index from the vector of indexes
	  Float_t pidx = Dvpions[wip];
	  ...
	  if ( hold1 < cutR )
	    {
	      //Calculate the number of Daughters of every Pion
	      numPionDau = pythia.event[pidx].daughter2() - pythia.event[pidx].daughter1() + 1;
	      
	      //Loop Over the Pion Daughters
      ...
      /*while ( dpts1.size() > 0) 
	{
      ...
      if(idbg>0) 
	cout<<" id mothers daughters pt y phi deltaR"<<endl;
